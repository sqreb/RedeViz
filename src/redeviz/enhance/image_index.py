...
def compute_image_ST_ref_dist(ST_bin_cnt, ST_bin_cnt_cutoff_arr, ref_norm_gene_bin_pct, fct, bg_fct_li=[0.75, 0.8, 0.9, 1.0]):
    ...
    ref_ST_dist_li = list()
    for bg_fct in bg_fct_li:
        tmp_ref_norm_gene_bin_pct = ref_norm_gene_bin_pct * bg_fct
        tmp_ref_ST_dist = tr.abs(tr.log2((tmp_ref_norm_gene_bin_pct+1e-3) / (norm_ST_bin_cnt+1e-3)))
        tmp_norm_ref_ST_dist = tr.sum(tmp_ref_ST_dist * fct.reshape([1, 1, 1, -1]), -1)
        ref_ST_dist_li.append(tmp_norm_ref_ST_dist)
    norm_ref_ST_dist, _ = tr.min(tr.stack(ref_ST_dist_li, -1), -1)
...
def infer_weighted_fct(gene_bin_pct, ref_norm_gene_bin_pct, bin_pheno_dist, embedding_resolution, UMI_per_spot, bin_size):
    ...
    for epoch in range(1000):
        ...
        norm_ref_ST_dist = compute_image_ST_ref_dist(ST_bin_cnt, ST_bin_cnt_cutoff_arr, ref_norm_gene_bin_pct, fct, bg_fct_li=[1.0])
...
def build_embedding_image_norm_index_main(args):
    f_in = args.input
    f_out = args.output
