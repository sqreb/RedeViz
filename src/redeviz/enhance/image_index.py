...
def select_gene_bin_cnt_gene(gene_bin_cnt, gene_name_arr, select_gene_li, device, Q=0.95):
    ...
    for index in range(gene_num):
        ...
        if(tr.sum(tmp_gene_bin_pct)==0):
            ...
            gene_bin_pct_cutoff_li.append(tr.quantile(tmp_gene_bin_pct[tmp_gene_bin_pct>0], Q))
...
def compute_image_ST_ref_dist(ST_bin_cnt, ST_bin_cnt_cutoff_arr, ref_norm_gene_bin_pct, fct):
    ...
    ref_norm_gene_bin_pct = ref_norm_gene_bin_pct
...
def infer_simulated_ST_bin_cnt_cutoff(gene_bin_pct, UMI_per_spot, bin_size, N_simu_sample = 1000, Q=0.95):
    ...
    for index in range(gene_num):
        ...
        if(tr.sum(tmp_gene_bin_cnt)==0):
            ...
            ST_bin_cnt_cutoff_li.append(tr.quantile(tmp_gene_bin_cnt[tmp_gene_bin_cnt>0], Q))
