...
def parse_enhance_args(enhance_subparser):
    ...
    parser_pretreat.add_argument("--leiden-resolution", type=float, dest="leiden_resolution",
    ...
    parser_pretreat_by_ST = enhance_subparser.add_parser('pretreat_by_ST', help='Pretreat by ST data help')
    parser_pretreat_by_ST.set_defaults(func=build_embedding_info_by_ST_main)
    parser_pretreat_by_ST.add_argument("--spot", type=str, dest="spot",
                        metavar="spot.tsv", required=True, help="ST data with TSV format.")
    parser_pretreat_by_ST.add_argument("--mask", type=str, dest="mask",
                        metavar="mask.npz", required=True, help="Cell mask data with npz format.")
    parser_pretreat_by_ST.add_argument("--gene-id-label", type=str, dest="gene_id_label",
                        metavar="gene_id_label", required=False, default=None, help="Gene ID label in scRNA-seq dataset of variable dataframe.")
    parser_pretreat_by_ST.add_argument("--x-index-label", type=str, dest="x_index_label",
                        metavar="x_index_label", required=False, default="spot_x_index", 
                        help="X index label: (default: x_index_label)")
    parser_pretreat_by_ST.add_argument("--y-index-label", type=str, dest="y_index_label",
                        metavar="y_index_label", required=False, default="spot_y_index", 
                        help="Y index label: (default: y_index_label)")
    parser_pretreat_by_ST.add_argument("--UMI-label", type=str, dest="UMI_label",
                        metavar="UMI_label", required=False, default="UMI", 
                        help="UMI label (default: UMI)")
    parser_pretreat_by_ST.add_argument("--embedding", type=str, dest="embedding",
                        metavar="embedding", required=False, default="tSNE", choices=["tSNE", "UMAP"], 
                        help="Embedding method [tSNE, UMAP] (default=tSNE)")
    parser_pretreat_by_ST.add_argument("--embedding-resolution", type=float, dest="embedding_resolution",
                        metavar="embedding_resolution", required=False, default=None, help="Embedding bin resolution [default value: 2.5 (Dim=2); 6 (Dim=3)]")
    parser_pretreat_by_ST.add_argument("--embedding-dim", type=int, dest="embedding_dim",
                        metavar="embedding_dim", required=False, default=2, choices=[2, 3], help="Embedding dim [2, 3] (default=2)")
    parser_pretreat_by_ST.add_argument("--n-neighbors", type=int, dest="n_neighbors",
                        metavar="n_neighbors", required=False, default=None, help="n_neighbors params in UMAP method")
    ...
    parser_pretreat_by_ST.add_argument("--gene-blacklist", type=str, dest="gene_blacklist",
                        metavar="gene_blacklist.txt", required=False, default=None, help="Genes need to remove from scRNA-seq data")
    parser_pretreat_by_ST.add_argument("--max-expr-ratio", type=float, dest="max_expr_ratio",
                        metavar="max_expr_ratio", required=False, default=0.05, help="Max gene expression filtering ratio [0, 1]")
    parser_pretreat_by_ST.add_argument("--cell-state-topN-gene", type=int, dest="cell_state_topN_gene",
                        metavar="cell_state_topN_gene", required=False, default=2, help="topN gene for filtering cell state (default=2)")
    parser_pretreat_by_ST.add_argument("--cell-state-topN-gene-max-ratio", type=float, dest="cell_state_topN_gene_max_ratio",
                        metavar="cell_state_topN_gene_max_ratio", required=False, default=0.3, help="Max gene expression ratio for topN gene in each cell state (default=0.3)")
    parser_pretreat_by_ST.add_argument("--no-HVG", dest="no_HVG", action="store_true")
    parser_pretreat_by_ST.add_argument("--merge-method", type=str, dest="merge_method",
                        metavar="merge_method", required=False, default="None", choices=["None", "log"], 
                        help="Merge method [None, log] (default=None)")
    parser_pretreat_by_ST.add_argument("--device-name", type=str, dest="device_name",
                        metavar="device_name", required=False, default="CPU", 
                        help="Device name (default=CPU)")
    parser_pretreat_by_ST.add_argument("-o", "--output", type=str, dest="output",
                        metavar="output", required=True, help="Output folder")
